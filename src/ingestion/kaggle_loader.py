...
def download_kaggle_dataset(dataset_id: str, output_dir: Optional[Path] = None) -> Path:
...
def load_primary_dataset(filepath: Optional[Path] = None) -> pd.DataFrame:
    ...
    df["kilometres"] = pd.to_numeric(
        df["kilometres"].astype(str).str.replace(",", "").str.replace(" km", ""),
        errors="coerce"
    )
...
def load_secondary_dataset(filepath: Optional[Path] = None) -> pd.DataFrame:
...
def deduplicate_listings(df: pd.DataFrame) -> pd.DataFrame:
    ...
    after_count = len(df)
    logger.info(f"Deduplication: {before_count} -> {after_count} records ({before_count - after_count} removed)")
