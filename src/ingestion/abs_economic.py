...
def fetch_abs_cpi() -> pd.DataFrame:
    ...
    try:
        ...
        time_periods = [
            dim["id"] for dim in data["structure"]["dimensions"]["observation"][0]["values"]
        ]
...
def deflate_prices(
    ...
    # Get base period CPI
    base_cpi = (
        cpi_df.loc[cpi_df["period"] == base_period, "cpi_index"].iloc[0]
        if base_period in cpi_df["period"].values
        else cpi_df["cpi_index"].iloc[-1]
    )

    # Map each price date to its quarter's CPI
    price_quarters = pd.PeriodIndex(price_dates, freq="Q")
    cpi_mapping = dict(
        zip(pd.PeriodIndex(cpi_df["period"], freq="Q"), cpi_df["cpi_index"], strict=False)
    )
