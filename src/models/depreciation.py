...
def exponential_decay(age: np.ndarray, initial_value: float, decay_rate: float) -> np.ndarray:
    ...
    """
    return initial_value * np.exp(-decay_rate * age)
...
def compute_retention_curve(
    ...
    # Calculate retention percentage (relative to age 0-1 median)
    new_price = curve_data.loc[curve_data["age"] <= 1, "median_price"].median()
